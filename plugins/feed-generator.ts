...
function getArticlePages() {
  return pages.filter(page => page.slug.startsWith('articles/')).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
}
...
export function feedGenerator(): Plugin {
  return {
    ...
    async closeBundle() {
      // Write sitemap.xml
      await writeFile(resolve(process.cwd(), 'dist', 'sitemap.xml'), renderSitemapXml())
      // Write RSS
      await writeFile(resolve(process.cwd(), 'dist', 'rss.xml'), renderRssXml())
      // Write Atom
      await writeFile(resolve(process.cwd(), 'dist', 'atom.xml'), renderAtomXml())
      // Write JSON Feed
      await writeFile(resolve(process.cwd(), 'dist', 'feed.json'), renderJsonFeed())
      console.log('✅ Generated sitemap.xml, rss.xml, atom.xml, feed.json in dist/')
    }
  }
} 